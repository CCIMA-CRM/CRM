...
app.controller('InsertLeadCtrl', function($scope, $location, $http, Session) {
    ...
    $scope.fileNameChanged = function(element) {
        $scope.$apply(function() {
            for (var i = 0; i < element.files.length; i++) {
                lead.newLead.files.push(element.files[i]);
            }
        });
        updateFileList();
    };
    ...
    function updateFileList() {
        var fileList = document.querySelector('.file-path');
        if (fileList) {
            fileList.value = lead.newLead.files.map(f => f.name).join(', ');
        }
    }

    lead.removeFile = function(index) {
        lead.newLead.files.splice(index, 1);
        updateFileList();
    };
